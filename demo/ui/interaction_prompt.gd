...
# the "players" group DemoPlayerController already adds its Actor to.
var _player_controller: DemoPlayerController
...
func _find_player_controller() -> void:
	for node in get_tree().get_nodes_in_group("players"):
		...
		if actor and actor.controller is DemoPlayerController:
